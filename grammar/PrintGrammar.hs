...
instance Print (Arg a) where
  prt i e = case e of
    Arg _ argmod id type_ -> prPrec i 0 (concatD [prt 0 argmod, prt 0 id, doc (showString ":"), prt 0 type_])
  prtList _ [] = (concatD [])
...
instance Print (Stmt a) where
  prt i e = case e of
    ...
    EStmt _ expr -> prPrec i 5 (concatD [prt 0 expr])
    ...
    VarDef _ id fulltype expr -> prPrec i 5 (concatD [doc (showString "def"), prt 0 id, doc (showString ":"), prt 0 fulltype, doc (showString "="), prt 1 expr])
    Ass _ id expr -> prPrec i 5 (concatD [prt 0 id, doc (showString "="), prt 1 expr])
    FnDef _ id fulltype args stmt -> prPrec i 4 (concatD [doc (showString "def"), prt 0 id, doc (showString ":"), prt 0 fulltype, doc (showString "("), prt 0 args, doc (showString ")"), prt 5 stmt])
    Cond _ expr stmt -> prPrec i 1 (concatD [doc (showString "if"), prt 0 expr, doc (showString ":"), prt 1 stmt])
    CondElse _ expr stmt1 stmt2 -> prPrec i 1 (concatD [doc (showString "if"), prt 0 expr, doc (showString ":"), prt 1 stmt1, doc (showString "else"), prt 1 stmt2])
    While _ expr stmt -> prPrec i 1 (concatD [doc (showString "while"), prt 0 expr, doc (showString ":"), prt 1 stmt])
    ForList _ id expr stmt -> prPrec i 1 (concatD [doc (showString "for"), prt 0 id, doc (showString "from"), prt 0 expr, doc (showString ":"), prt 1 stmt])
    ForRange _ id expr1 expr2 stmt -> prPrec i 1 (concatD [doc (showString "for"), prt 0 id, doc (showString "from"), prt 0 expr1, doc (showString "to"), prt 0 expr2, doc (showString ":"), prt 1 stmt])
    Composition _ stmt1 stmt2 -> prPrec i 0 (concatD [prt 0 stmt1, doc (showString ";"), prt 1 stmt2])
...
instance Print (Expr a) where
  prt i e = case e of
    ...
    ELitInt _ n -> prPrec i 8 (concatD [prt 0 n])
    ELitTrue _ -> prPrec i 8 (concatD [doc (showString "true")])
    ELitFalse _ -> prPrec i 8 (concatD [doc (showString "false")])
    EString _ str -> prPrec i 8 (concatD [prt 0 str])
    ELitFloat _ d -> prPrec i 8 (concatD [prt 0 d])
    Neg _ expr -> prPrec i 7 (concatD [doc (showString "-"), prt 8 expr])
    Not _ expr -> prPrec i 7 (concatD [doc (showString "!"), prt 8 expr])
    ...
    ELambda _ fulltype args stmt -> prPrec i 1 (concatD [doc (showString "lambda"), doc (showString ":"), prt 0 fulltype, doc (showString "("), prt 0 args, doc (showString ")"), doc (showString "->"), prt 1 stmt])
    ERand _ expr -> prPrec i 1 (concatD [doc (showString "random"), doc (showString "from"), prt 2 expr])
    ERandDist _ expr1 expr2 -> prPrec i 1 (concatD [doc (showString "random"), doc (showString "from"), prt 2 expr1, doc (showString "distribution"), prt 2 expr2])
    EProb _ stmt expr -> prPrec i 1 (concatD [doc (showString "probability"), doc (showString "of"), prt 1 stmt, doc (showString "satisfying"), prt 2 expr])
    EProbSamp _ expr1 stmt expr2 -> prPrec i 1 (concatD [doc (showString "probability"), doc (showString "tested"), prt 2 expr1, doc (showString "times"), doc (showString "of"), prt 1 stmt, doc (showString "satisfying"), prt 2 expr2])
    EList _ exprs -> prPrec i 0 (concatD [doc (showString "<<"), prt 0 exprs, doc (showString ">>")])
    EArray _ exprs -> prPrec i 0 (concatD [doc (showString "["), prt 0 exprs, doc (showString "]")])
    EApp _ id exprs -> prPrec i 0 (concatD [prt 0 id, doc (showString "("), prt 0 exprs, doc (showString ")")])
  prtList _ [] = (concatD [])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString ","), prt 0 xs])
...
instance Print (OrOp a) where
  prt i e = case e of
    ...
    OrSym _ -> prPrec i 0 (concatD [doc (showString "||")])

instance Print (AndOp a) where
  prt i e = case e of
    And _ -> prPrec i 0 (concatD [doc (showString "and")])
    AndSym _ -> prPrec i 0 (concatD [doc (showString "&&")])
